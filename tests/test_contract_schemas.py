...
def test_canonical_fixture_validates_against_contract(definition: object) -> None:
    fixture_path = FIXTURE_ROOT / f"{definition.name}.json"  # type: ignore[attr-defined]
    payload = json.loads(fixture_path.read_text(encoding="utf-8"))

    instance = definition.model.model_validate(payload)  # type: ignore[attr-defined]

    assert instance.schema_version == SCHEMA_VERSION
    assert instance.kind == definition.name  # type: ignore[attr-defined]


@pytest.mark.parametrize("definition", CONTRACTS, ids=lambda item: item.name)
def test_contracts_reject_unknown_fields(definition: object) -> None:
    fixture_path = FIXTURE_ROOT / f"{definition.name}.json"  # type: ignore[attr-defined]
    payload = json.loads(fixture_path.read_text(encoding="utf-8"))
    payload["unexpected"] = True

    with pytest.raises(ValidationError):
        definition.model.model_validate(payload)  # type: ignore[attr-defined]
...
@pytest.mark.parametrize("definition", CONTRACTS, ids=lambda item: item.name)
def test_v1_compatibility_fixture_round_trips(definition: object) -> None:
    fixture_path = FIXTURE_ROOT / f"{definition.name}.json"  # type: ignore[attr-defined]
    payload = json.loads(fixture_path.read_text(encoding="utf-8"))
    instance = definition.model.model_validate(payload)  # type: ignore[attr-defined]
    serialized = instance.model_dump(mode="json", exclude_none=True)

    reloaded = definition.model.model_validate(serialized)  # type: ignore[attr-defined]
